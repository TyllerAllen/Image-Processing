...
public class ThickeningOperation implements Operation {
	...
	public ThickeningOperation(int iterations, ImageNode source) {
		this.source = source;
		this.iterations= iterations;
		this.original= source.getParent().getObjects().get(0).getThumb();
		slider = new JSlider(1, 200, iterations);
		p.add(new JLabel("Number of Thickening iterations"));
		...
		tf.getDocument().addDocumentListener(new DocumentListener() {
			...
			private void extracted() {
				...
				try {
					...
					if (parseInt != ThickeningOperation.this.iterations) {
		...
		slider.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				if (nv != ThickeningOperation.this.iterations) {

					ThickeningOperation.this.iterations = slider.getValue();
					tf.setText(String.format("%d", ThickeningOperation.this.iterations));
					thickening(slider.getValue());
	...
	private void thickening(int in) {
		this.iterations=in;
