...
public class PercentFeature implements Feature {

	private FlowPanel p;
	private ArrayList<Double> rv;

	private PercentFeature(ArrayList<Double> rv) {

		this.rv = new ArrayList<Double>();
		p = new FlowPanel('x');
		p.add(new JLabel("Black percent vector: "));
	...
	public static FeatureFactory <PercentFeature>factory = new FeatureFactory<PercentFeature>() {

		@Override
		public PercentFeature createFeature(ImageObject o) {
			
			
			ArrayList<Double> rv = o.getMap().divideInto9AndGetPercentVector();
			

			return new PercentFeature(rv);
